...
- **Edit mode:** place components and zones on the grid, draw connections, add / duplicate / delete steps, drag components and zones anywhere — past the grid's far edge grows it, past the origin re-bases the scene — add routing waypoints by double-clicking a pipe and remove them by double-clicking the waypoint, change a component's size / shape / colour / icon, set its pinned label and anchor, rename zone and pipe labels. Deletes show what they cascade to first, edits are undo/redo-able (⌘Z / ⇧⌘Z), and any object — or the whole flow — can be edited as raw JSON when no form covers what you need. **Save to file** writes it back to the flow's JSON (or Copy JSON to paste it elsewhere)
- **Pinned component labels:** a component can carry a name that stays on screen, anchored at any of nine points against it and tinted with its own colour, for when a diagram has to be read as a still rather than hovered. A playback-bar toggle brings them up
- **Import:** a flow a colleague sent, as a `.json` file or pasted text, validated against the schema and written into `public/flows/custom/` under a name you choose
...
- **Viewer controls:** zoom in and out a step at a time (buttons or `+` / `-`), with a readout that says how far in you are and returns you to the opening framing when pressed; hide the pipes, or just the chips naming them, when either crowds a screenshot; slide the step list off screen to give the diagram the whole window, and follow a line creeping across the step that is playing so a step that animates nothing still shows progress
