...
- **Viewer controls:** zoom in and out a step at a time (buttons or `+` / `-`), with a readout that says how far in you are and returns you to the opening framing when pressed; hide the pipes, or just the chips naming them, when either crowds a screenshot; slide the step list off screen to give the diagram the whole window
